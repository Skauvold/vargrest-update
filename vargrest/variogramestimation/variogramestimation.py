...
from vargrest.variogramestimation.empiricalvariogram import (
    _estimate_variogram_np_3d_dense,
    _estimate_variogram_np_3d_random,
    _estimate_variogram_np_3d_sparse,
)
...
class NonparametricVariogramEstimate:
    def __init__(
        ...
        counts: np.ndarray,
        ndims: int,
        gridres: Tuple[float, float, float],
    ):
        super().__init__()
        self._varmap = varmap
        self._counts = counts
...
class ParametricVariogramEstimate:
    def __init__(
        ...
        self._description = "{}D {} variogram estimate ({} parameters)".format(
            ndims, family, len(params)
        )
    ...
    def variogram_function(self) -> Callable:
        return AnisotropicVariogram(
            family=self._family, nug=self._nugget
    ...
    def polished_parameters(self):
        ...
        if (
            polished_parameters["r_major"]["value"]
            < polished_parameters["r_minor"]["value"]
        ):
...
class VariogramEstimator:
    ...
    def make_variogram_map_xyz(
        ...
        if sampling == "dense":
            varmap, counts = _estimate_variogram_np_3d_dense(
                self._data, lag_x, lag_y, lag_z, sub_sampling
            )
        elif sampling == "random":
            varmap, counts = _estimate_variogram_np_3d_random(
                self._data,
                lag_x,
                lag_y,
                lag_z,
            ...
            varmap, counts = _estimate_variogram_np_3d_sparse(
                self._data, lag_step_x, lag_step_y, lag_step_z
            )
        else:
            assert False
        return NonparametricVariogramEstimate(
            varmap, counts, ndims=3, gridres=self._grid_resolution
        )

    def estimate_parametric_variogram_xyz(
        self,
        varmap: NonparametricVariogramEstimate,
        family: Union[VariogramType, str],
        nugget: bool,
        sigma_wt: Optional[float] = None,
    ) -> ParametricVariogramEstimate:
        ...
        popt, quality = self._find_optimum(
            varmap,
            vfunc,
            av.get_variogram_jacobian(),
            parameter_names,
            sigma_wt,
            normalize=True,
        )
        ...
        par_est_dict = {
            par_name: par_value for (par_name, par_value) in zip(vfunc_args[1:], popt)
        }

        # Return estimation result as an object
        return ParametricVariogramEstimate(
            family=family,
            params=par_est_dict,
            quality=quality,
            ndims=3,
    ...
    def generate_3d_slice_image(self, dir_name: str, file_name: str):
        ...
        fig = go.Figure()
        xx, yy = np.meshgrid(
            np.arange(0, self._data.shape[0]) * self._grid_resolution[0],
        ...
        for i in range(len(fig.data)):
            s = dict(method="restyle", args=["visible", [False] * len(fig.data)])
            s["args"][1][i] = True
            steps.append(s)
        ...
        fig.write_html(
            os.path.join(dir_name, file_name) + ".html", include_plotlyjs="cdn"
        )

    def _find_optimum(
        self, var_map, var_func, jac_func, parameter_names, sigma_wt, normalize=False
    ):
        ...
        for ind, pn in enumerate(parameter_names):
            par_guess[ind] = self._parameter_guess(
                pn, varmap_shape, varmap_mean, varmap_max
            )
            lb, ub = self._parameter_bounds(pn, varmap_shape, varmap_mean, varmap_max)
            par_lbounds[ind] = lb
            par_ubounds[ind] = ub
        par_ubounds = np.maximum(
            par_ubounds, par_lbounds + 1e-12
        )  # Make sure bounds are well-defined

        # Identify azimuth by 'angle sweep' method
        azimuth_opt = find_dominant_direction(
            var_map.variogram_map_values(normalize), self._grid_resolution
        )
        ...
        if normalize:
            for ind, pn in enumerate(parameter_names):
                if pn in ["sig", "std", "sigma"]:
                    _popt[ind] *= np.sqrt(
                        np.nanmean(var_map.variogram_map_values(normalized=False))
                    )  # Un-normalize standard deviation estimate

        return _popt, _quality

    def _parameter_guess(
        self,
        parname: str,
        varmap_shape: Tuple[int, int, int],
        varmap_mean: float,
        varmap_max: float,
    ) -> float:
