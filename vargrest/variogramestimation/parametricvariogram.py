...
class AnisotropicVariogram:
    ...
    def __init__(self, family: Union[VariogramType, str], nug: bool = False):
        ...
        if nug is True:
            ...
            self._g_func = {
                ...
                VariogramType.GeneralExponential: lambda d, r: _general_exponential_corr(
                    d, r, self.GEN_EXP_PWR
                ),
            ...
            self._dg_func = {
                ...
                VariogramType.GeneralExponential: lambda d, r: _d_general_exponential_corr(
                    d, r, self.GEN_EXP_PWR
                ),
    ...
    def _distance(self, dx, dy, dz, rx, ry=1.0, rz=1.0, azi=0.0, dip=0.0):
        ...
        f3 = 1 / rz**2
        self._txx = (
            cos_rot * cos_rot * cos_dip * cos_dip * f1
            + sin_rot * sin_rot * f2
            + cos_rot * cos_rot * sin_dip * sin_dip * f3
        )
        self._tyy = (
            sin_rot * sin_rot * cos_dip * cos_dip * f1
            + cos_rot * cos_rot * f2
            + sin_rot * sin_rot * sin_dip * sin_dip * f3
        )
        self._tzz = sin_dip * sin_dip * f1 + cos_dip * cos_dip * f3
        self._txy = 2 * (
            cos_dip * cos_dip * cos_rot * sin_rot * f1
            - sin_rot * cos_rot * f2
            + sin_dip * sin_dip * cos_rot * sin_rot * f3
        )
        self._txz = 2 * (
            cos_rot * cos_dip * sin_dip * f1 - cos_rot * cos_dip * sin_dip * f3
        )
        self._tyz = 2 * (
            sin_rot * cos_dip * sin_dip * f1 - sin_rot * cos_dip * sin_dip * f3
        )
    ...
    def _d_distance2(self, dx, dy, dz, rx, ry, rz, azi, dip):
        ...
        dt = np.array(
            [
                ...
                # d azi
                [
                    -2 * sin(azi) * sin(dip) ** 2 * cos(azi) / rz**2
                    + 2 * sin(azi) * cos(azi) / ry**2
                    - 2 * sin(azi) * cos(azi) * cos(dip) ** 2 / rx**2,
                    2 * sin(azi) * sin(dip) ** 2 * cos(azi) / rz**2
                    - 2 * sin(azi) * cos(azi) / ry**2
                    + 2 * sin(azi) * cos(azi) * cos(dip) ** 2 / rx**2,
                    0,
                    -(2 * sin(dip) ** 2 / rz**2 - 2 / ry**2 + 2 * cos(dip) ** 2 / rx**2)
                    * sin(azi) ** 2
                    + (
                        2 * sin(dip) ** 2 / rz**2
                        - 2 / ry**2
                ...
                # d dip
                [
                    2 * sin(dip) * cos(azi) ** 2 * cos(dip) / rz**2
                    - 2 * sin(dip) * cos(azi) ** 2 * cos(dip) / rx**2,
                    2 * sin(azi) ** 2 * sin(dip) * cos(dip) / rz**2
                    - 2 * sin(azi) ** 2 * sin(dip) * cos(dip) / rx**2,
                    -2 * sin(dip) * cos(dip) / rz**2 + 2 * sin(dip) * cos(dip) / rx**2,
                    (4 * sin(dip) * cos(dip) / rz**2 - 4 * sin(dip) * cos(dip) / rx**2)
                    * sin(azi)
                    * cos(azi),
                    -(2 / rz**2 + 2 / rx**2) * sin(dip) ** 2 * cos(azi)
                    + (2 / rz**2 + 2 / rx**2) * cos(azi) * cos(dip) ** 2,
                    -(2 / rz**2 + 2 / rx**2) * sin(azi) * sin(dip) ** 2
                    + (2 / rz**2 + 2 / rx**2) * sin(azi) * cos(dip) ** 2,
                ],
            ]
        )
        h = np.stack(
    ...
    def _jac(self, dxyz, rx, ry, rz, azi, dip, std):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            d_other = (
                ...
                * self._dg_func(d, 1.0)
                * 0.5
                * self._d_distance2(dx, dy, dz, rx, ry, rz, azi, dip)
                / d
            )
    ...
    def _get_exponential_variogram_function(self, nug: bool = False) -> Callable:
        if nug:
            return lambda dxyz, rx, ry, rz, azi, dip, std, sng: (sng**2) + (std**2) * (
                ...
                - _exponential_corr(
                    self._distance(
                        dxyz[0, :], dxyz[1, :], dxyz[2, :], rx, ry, rz, azi, dip
                    ),
                    1.0,
                )
            )
    ...
    def _get_general_exponential_variogram_function(
        self, nug: bool = False
    ) -> Callable:
