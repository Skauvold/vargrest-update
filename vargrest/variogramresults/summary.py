...
def summarize(
    ...
    meta_data: Dict[SummaryDataType, Union[str, int, float]],
) -> ExecutionSummary:
    polished = pe.polished_parameters()
    polished_flat = {
        f'{k}[{polished[k]["unit"]}]': polished[k]["value"] for k in polished.keys()
    }
    summary = {
...
def conclude(
    ...
    vd.plot_crop_box(
        save_figure=True, dir_name=qc_dir, file_name=fn_template + "_crop_"
    )

    # Pickle variogram estimation data
    if full_qc is True:
        pickle.dump(
            (ve, ne), open(os.path.join(qc_dir, fn_template + "_data_.pkl"), "wb")
        )
