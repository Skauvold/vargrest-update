...
class SlicePlot:
    ...
    def add_true_variogram(self, rx, ry, rz, azi, true_sd, data_sd):
        ...
        nx, ny, nz = self._shape
        dx, dy, dz = self._resolution
        tru_v = variogram.SphericalVariogram(rx, ry, rz, azi).create_corr_array(
            nx, dx, ny, dy, nz, dz
        )
        tru_v = tru_v[
            nx // 2 + 1 : nx // 2 + nx + 1,
    ...
    def _add_slices(self, grid, *args, scale=False, **kwargs):
        ...
        self._axes[0, 0].plot(
            *_filter_nan(xax, _scale(grid[:, ny // 2, nz // 2])), *args, **kwargs
        )
        ...
        self._axes[0, 1].plot(
            *_filter_nan(xax, _scale(grid[nx // 2, :, nz // 2])), *args, **kwargs
        )
        ...
        self._axes[1, 0].plot(
            *_filter_nan(xax, _scale(grid[nx // 2, ny // 2, :])), *args, **kwargs
        )
