...
def visualize_2d_variogram_map(
    varmap: np.ndarray,
    ...
    if varmap.ndim != 2:
        raise Exception(
            "Variogram map must have 2 dimensions, not {}.".format(varmap.ndim)
        )

    n_x, n_y = varmap.shape
    dx, dy = res
    h_ext = (-0.5 * n_x * dx, 0.5 * n_x * dx, -0.5 * n_y * dy, 0.5 * n_y * dy)
...
def visualize_3d_variogram_map(
    ax,
    varmap: np.ndarray,
    ...
    if varmap.ndim != 3:
        raise Exception(
            "Variogram map must have 3 dimensions, not {}.".format(varmap.ndim)
        )
    ...
    kwargs = dict(extent=h_ext, origin="lower")
    if clims is not None:
        kwargs["vmin"] = clims[0]
