...
class VariogramPlot:
    ...
    def _set_empirical_variogram_plot(self, ne: NonparametricVariogramEstimate):
        ...
        img2 = visualize_3d_variogram_map(
            ...
            vm / np.nanmax(vm),
            res_3d,
            (0.0, self._red_threshold),
            red_threshold=self._red_threshold,
        )
        self._fig.colorbar(img2, ax=ax2)
        ax2.set_title("Empirical variogram (scaled)")

    def _set_parametric_variogram_plot(
        self, ne: NonparametricVariogramEstimate, pe: ParametricVariogramEstimate
    ):
        ...
        img2 = ax2.imshow(
            varest[:, :, z_plot].T / sill,
            vmax=self._red_threshold,
            vmin=0.0,
            extent=(dx * xmin, dx * xmax, dy * ymin, dy * ymax),
            origin="lower",
        )
